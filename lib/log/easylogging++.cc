...
#ifdef CUSTUM_EDITOR_JAYDUK

std::string make_coloured(std::string& msg)
{
    const static std::string reset = "\033[0m";
    const static std::map<std::string, std::string> colorMap = {
        {"[INFO]", "\033[32m"},     // Green color for INFO level
        {"[WARNING]", "\033[33m"},  // Yellow color for WARNING level
        {"[ERROR]", "\033[31m"},    // Red color for ERROR level
        {"[TRACE]", "\033[00m"},    // white color for ERROR level
        {"[DEBUG]", "\033[34m"},    // Red color for ERROR level
        // Add more color mappings for different levels here
    };

    for (auto&& [key, value] : colorMap)
    {
        auto start_pos = msg.find(key);
        if (start_pos != std::string::npos)
        {
            std::string target_str = value + key + reset;
            target_str.resize(18, ' ');
            msg.replace(start_pos, key.size(), target_str);
...
base::type::string_t DefaultLogBuilder::build(const LogMessage* logMessage, bool appendNewLine) const
{
    ...
    {
        // DateTime
        base::utils::Str::replaceFirstWithEscape(logLine, base::consts::kDateTimeFormatSpecifier, base::utils::DateTime::getDateTime(logFormat->dateTimeFormat().c_str(), &tc->subsecondPrecision(logMessage->level())));
    }
